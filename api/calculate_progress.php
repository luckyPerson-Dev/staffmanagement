 ...
 * Calculate progress percentage (AJAX endpoint)
 * 
 * NEW LOGIC:
...
// 2) If missed day is checked: Set all to 0, skip further logic
if ($is_missed) {
    $final_progress = 0;
    $ticket_progress = 0;
    $group_progress = 0;
    ...
    // Get group ratios from settings
    ...
    // If groups_status is passed as array, process it
    if (!empty($groups_status) && is_array($groups_status)) {
    ...
    if ($final_progress > $max_allowed) {
        error_log("Progress exceeds maximum allowed: date=$date, progress=$final_progress, max=$max_allowed");
    }
...
// Use final_progress for response
$progress = $final_progress;

response_json([
    'success' => true,
    'progress_percent' => $progress,
    'ticket_progress' => round($ticket_progress, 2),
    'group_progress' => round($group_progress, 2),
    'merged' => isset($merged) ? round($merged, 2) : 0,
